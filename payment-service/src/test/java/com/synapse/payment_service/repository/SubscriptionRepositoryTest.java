...
class SubscriptionRepositoryTest {
    ...
    void findActiveSubscriptionsDueForRenewal_shouldReturnOnlyExactDateMatches() {
        ...
        Subscription exactDateSubscription = createActiveSubscription(targetDate.atStartOfDay().atZone(ZonedDateTime.now().getZone()));
        
        // 과거 날짜에 만료된 구독 (조회되지 않아야 함)
        Subscription pastDateSubscription = createActiveSubscription(pastDate.atStartOfDay().atZone(ZonedDateTime.now().getZone()));
        
        // 미래 날짜에 만료되는 구독 (조회되지 않아야 함)
        Subscription futureDateSubscription = createActiveSubscription(futureDate.atStartOfDay().atZone(ZonedDateTime.now().getZone()));
    ...
    void findActiveSubscriptionsDueForRenewal_shouldNotReturnPastDueSubscriptions() {
        // given
        // 과거 여러 날짜에 만료된 구독들
        Subscription pastSubscription1 = createActiveSubscription(pastDate.minusDays(5).atStartOfDay().atZone(ZonedDateTime.now().getZone()));
        Subscription pastSubscription2 = createActiveSubscription(pastDate.minusDays(10).atStartOfDay().atZone(ZonedDateTime.now().getZone()));
        Subscription pastSubscription3 = createActiveSubscription(pastDate.atStartOfDay().atZone(ZonedDateTime.now().getZone()));
    ...
    void findActiveSubscriptionsDueForRenewal_shouldNotReturnFutureSubscriptions() {
        // given
        // 미래 여러 날짜에 만료되는 구독들
        Subscription futureSubscription1 = createActiveSubscription(futureDate.plusDays(1).atStartOfDay().atZone(ZonedDateTime.now().getZone()));
        Subscription futureSubscription2 = createActiveSubscription(futureDate.plusDays(10).atStartOfDay().atZone(ZonedDateTime.now().getZone()));
        Subscription futureSubscription3 = createActiveSubscription(futureDate.atStartOfDay().atZone(ZonedDateTime.now().getZone()));
    ...
    void findActiveSubscriptionsDueForRenewal_shouldNotReturnNonAutoRenewSubscriptions() {
        ...
        Subscription subscription = createActiveSubscription(targetDate.atStartOfDay().atZone(ZonedDateTime.now().getZone()));
    ...
    void findActiveSubscriptionsDueForRenewal_shouldReturnAllValidSubscriptions() {
        // given
        Subscription subscription1 = createActiveSubscription(targetDate.atStartOfDay().atZone(ZonedDateTime.now().getZone()));
        Subscription subscription2 = createActiveSubscription(targetDate.atTime(12, 30).atZone(ZonedDateTime.now().getZone()));
        Subscription subscription3 = createActiveSubscription(targetDate.atTime(23, 59).atZone(ZonedDateTime.now().getZone()));
    ...
    void findByStatusInAndExpiresAtBefore_shouldReturnCanceledExpiredSubscriptions() {
        ...
        Subscription canceledNotExpiredSubscription = createSubscriptionWithStatus(SubscriptionStatus.CANCELED, futureTime);
        ...
        subscriptionRepository.saveAll(List.of(canceledExpiredSubscription, canceledNotExpiredSubscription, activeExpiredSubscription));
    ...
    void findByStatusInAndExpiresAtBefore_shouldReturnPaymentFailedExpiredSubscriptions() {
        ...
        Subscription paymentFailedExpiredSubscription = createSubscriptionWithStatus(SubscriptionStatus.PAYMENT_FAILED, pastTime);
        
        // PAYMENT_FAILED 상태이지만 아직 만료되지 않은 구독 (조회되지 않아야 함)
        Subscription paymentFailedNotExpiredSubscription = createSubscriptionWithStatus(SubscriptionStatus.PAYMENT_FAILED, futureTime);
    ...
    void findByStatusInAndExpiresAtBefore_shouldReturnBothCanceledAndPaymentFailedExpiredSubscriptions() {
        ...
        subscriptionRepository.saveAll(List.of(canceledExpired1, canceledExpired2, paymentFailedExpired1, paymentFailedExpired2));
    ...
    void findByStatusInAndExpiresAtBefore_shouldNotReturnCurrentOrFutureSubscriptions() {
        ...
        Subscription canceledCurrentSubscription = createSubscriptionWithStatus(SubscriptionStatus.CANCELED, currentTime);
